...
function printSyncSummary(results: SyncResult[]) {
  ...
  statuteResults.forEach((result) => {
    ...
    console.log(
      `     Total: ${result.totalProcessed} | Success: ${result.successCount} | Failed: ${result.failureCount}`,
    )
  ...
  if (hasAnyFailures) {
    ...
    console.log(
      "Failed documents have been logged with full details for investigation.\n",
    )
  } else {
...
export const runSetup = async (startYear?: number) => {
  ...
  try {
    ...
    console.log(
      `[SETUP] Total setup time: ${totalTime}ms (${(totalTime / 1000 / 60).toFixed(2)} minutes)`,
    )
